...
final class ChatClient {
    ...
    public static void main(String[] args) {
        ...
        */
        ChatClient client;
        if (args.length == 3) {
            client = new ChatClient(args[2], Integer.parseInt(args[1], 10), args[0]);
            client.start();
            ...
            client = new ChatClient("localhost", Integer.parseInt(args[1], 10), args[0]);
            client.start();
            System.out.println("Connection accepted " + client.socket.getInetAddress().getHostName() + "/" +
                                       client.socket.getInetAddress().getHostAddress() + ":" + client.socket.getPort());
        ...
        ChatMessage m = new ChatMessage(in);
        while (m.getType() != 1) {
            Date now = new Date();
            SimpleDateFormat hh = new SimpleDateFormat("HH:mm:ss");
            //System.out.println(hh.format(now) + " " + client.username + ": " + in);
            client.sendMessage(m);
            in = scan.nextLine();
            m = new ChatMessage(in);
        }
